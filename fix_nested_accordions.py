...
# Common code for mapping and state
common_setup = """
...
roles_code = """window.abrirModalEditarPermisos = function(idRol) {
                ...
                for (const masterName of Object.keys(jerarquia)) {
                    ...
                    for (const subName of Object.keys(subs)) {
                        ...
                        htmlSubs += `
                            <div class="bg-white border-b border-slate-100 last:border-0 overflow-hidden transition-all">
                                <div class="flex items-center justify-between px-6 py-4 cursor-pointer hover:bg-slate-50 transition-colors" onclick="toggleAcordeonSub('${subKey}')">
                    ...
                    htmlMaster += `
                        <div class="mb-4 rounded-xl bg-white border border-slate-200 shadow-sm overflow-hidden transition-all">
                            <div class="flex items-center justify-between px-6 py-4 cursor-pointer transition-colors ${isMasterOpen ? 'bg-slate-50/50 border-b border-slate-100' : 'hover:bg-slate-50'}" onclick="toggleAcordeonMaster('${masterName}')">
                                ...
                                <div class="flex items-center gap-4">
                                    ...
                                    <div class="flex items-center gap-2 text-[12px] font-bold text-slate-500" onclick="event.stopPropagation()">
                                        <button type="button" onclick="seleccionarTodoMaster('${masterName}', true)" class="hover:text-slate-900 transition-colors">Todo</button>
                                        <button type="button" onclick="seleccionarTodoMaster('${masterName}', false)" class="hover:text-slate-900 transition-colors">Nada</button>
                                    </div>
                    ...
                    <div class="flex flex-col gap-4">
                        <div class="flex items-center gap-4 border border-slate-200 rounded-2xl p-1.5 mb-2 bg-white shadow-sm">
                            ...
                            <div class="flex items-center gap-3 px-4 py-1 text-[13px] font-bold text-slate-600">
                                <button type="button" onclick="seleccionarTodoGlobal(true)" class="hover:text-slate-900 transition-colors">Todo</button>
                                <span class="w-px h-3 bg-slate-300"></span>
                                <button type="button" onclick="seleccionarTodoGlobal(false)" class="hover:text-slate-900 transition-colors">Nada</button>
                            </div>
                            <div class="w-px h-6 bg-slate-200"></div>
            ...
            modalRoot.innerHTML = `
                ...
                <div id="permisosCard" class="relative bg-slate-50/90 backdrop-blur-md rounded-[32px] w-full max-w-[950px] max-h-[90vh] shadow-[0_20px_60px_rgb(0,0,0,0.15)] border border-slate-200/50 flex flex-col overflow-hidden z-[9999] transition-all m-auto">
                    <div class="flex items-center justify-between px-8 py-6 shrink-0">
                        ...
                        <button type="button" onclick="cerrarModalEditarPermisos()" class="w-8 h-8 flex items-center justify-center rounded-full bg-slate-100 hover:bg-slate-200 text-slate-500 transition-colors cursor-pointer" aria-label="Cerrar modal">
...
print("Roles Modal successfully upgraded to nested accordions!")
