    ...
    try {
        ...
        try {
            const [grupos] = await pool.query(`
                SELECT
                    c.id_conversacion,
                    c.nombre_grupo   AS nombre_contacto,
                    c.id_orden,
                    'grupo'          AS tipo,
                    NULL             AS id_contacto,
                    NULL             AS imagenUrl_contacto,
                    ...
                    (SELECT COUNT(*) FROM mensajes m2
                     WHERE m2.id_conversacion = c.id_conversacion
                       AND m2.leido = 0
                       AND m2.id_persona_envia != ?)                     AS mensajes_no_leidos,
                    (SELECT COUNT(*) FROM conversaciones_miembros
                     WHERE id_conversacion = c.id_conversacion)          AS total_miembros,
                    (SELECT MAX(m3.fecha_envio) FROM mensajes m3
                     WHERE m3.id_conversacion = c.id_conversacion)       AS ultima_actividad
                FROM conversaciones c
                JOIN conversaciones_miembros cm
                     ON cm.id_conversacion = c.id_conversacion
                WHERE c.tipo = 'grupo'
                  AND cm.id_Perfil_Persona = ?
                ORDER BY ultima_actividad DESC
            `, [personaId, personaId]);

            for (const g of grupos) {
                if (!g.ultimo_mensaje) g.ultimo_mensaje = 'Sin mensajes aún';
                conversaciones.push(g);
            }
        } catch (errGrupos) {
            console.warn('[Mensajeria] No se pudieron cargar grupos (tabla puede no existir):', errGrupos.message);
        }
        // ─────────────────────────────────────────────────────────────────────
...
router.get('/conversacion/:idConversacion/mensajes', async (req, res) => {
    try {
        ...
        // Verificar si la conversación es de grupo para evitar usar el SP 1:1
        let esGrupo = false;
        try {
            const [[convInfo]] = await pool.query(
                `SELECT tipo FROM conversaciones WHERE id_conversacion = ? LIMIT 1`,
                [conversacionId]
            );
            esGrupo = convInfo?.tipo === 'grupo';
        } catch (e) { /* continuar con SP */ }

        let mensajes = [];
        if (esGrupo) {
            // Para grupos: query directa con es_remitente_actual calculado en SQL
            const [grupoMsgs] = await pool.query(
                `SELECT m.*,
                        IF(m.id_persona_envia = ?, 1, 0) AS es_remitente_actual,
                        0 AS veces_editado,
                        0 AS borrado_para_todos,
                        m.contenido AS contenido_editado
                 FROM mensajes m
                 WHERE m.id_conversacion = ?
                 ORDER BY m.fecha_envio ASC`,
                [personaId, conversacionId]
            );
            mensajes = grupoMsgs || [];
        } else {
        ...
        // ── Enriquecer con nombre_emisor si la conversación es de grupo ──────
        try {
            if (esGrupo && mensajes.length > 0) {
                const emisoresIds = [...new Set(mensajes.map(m => m.id_persona_envia).filter(Boolean))];
                if (emisoresIds.length > 0) {
                    const [nombreRows] = await pool.query(
                        `SELECT id_Perfil_Persona,
                                TRIM(CONCAT(IFNULL(nombre_Persona,''), ' ', IFNULL(apellido_Persona,''))) AS nombre
                         FROM Personas WHERE id_Perfil_Persona IN (?)`,
                        [emisoresIds]
                    );
                    const nombreMap = {};
                    nombreRows.forEach(r => { nombreMap[r.id_Perfil_Persona] = r.nombre; });
                    mensajes = mensajes.map(m => ({
                        ...m,
                        nombre_emisor: nombreMap[m.id_persona_envia] || null
                    }));
                }
            }
        } catch (errNombre) {
            console.warn('[Mensajeria] nombre_emisor grupo:', errNombre.message);
        }
        // ─────────────────────────────────────────────────────────────────────
...
router.post('/upload', upload.single('file'), async (req, res) => {
    try {
        ...
        const r2Vars = {
            ...
            R2_SECRET_ACCESS_KEY: process.env.R2_SECRET_ACCESS_KEY || process.env.R2_SECRET_KEY || process.env.R_SECRET_ACCESS_KEY,
...
router.post('/enviar', async (req, res) => {
    try {
        ...
        // Verificar si es conversación de grupo → INSERT directo (SP no aplica para grupos)
        let esGrupoConv = false;
        try {
            const [[convRow]] = await pool.query(
                `SELECT tipo FROM conversaciones WHERE id_conversacion = ? LIMIT 1`,
                [conversacionId]
            );
            esGrupoConv = convRow?.tipo === 'grupo';
        } catch (e) { /* ignorar */ }

        let mensaje = null;
        if (esGrupoConv) {
            // Para grupos: INSERT directo sin id_persona_recibe
            const [insert] = await pool.query(
                `INSERT INTO mensajes (id_conversacion, id_persona_envia, id_persona_recibe, contenido, fecha_envio, leido, borrado_por_emisor, borrado_por_receptor)
                 VALUES (?, ?, NULL, ?, CURRENT_TIMESTAMP, 0, 0, 0)`,
                [conversacionId, personaEnviaId, contenidoStr]
            );
            const [rows] = await pool.query('SELECT * FROM mensajes WHERE id_mensaje = ?', [insert.insertId]);
            mensaje = rows && rows[0] ? rows[0] : null;
        } else {
            // Para 1:1: usar el stored procedure existente (compatibilidad)
...
// =========================================================
// MIEMBROS DE GRUPO
// GET /mensajeria/grupo/:idConversacion/miembros
// =========================================================
router.get('/grupo/:idConversacion/miembros', async (req, res) => {
    try {
        const { idConversacion } = req.params;
        const [rows] = await pool.query(`
            SELECT
                cm.id_Perfil_Persona,
                cm.rol,
                cm.fecha_union,
                TRIM(CONCAT(IFNULL(p.nombre_Persona,''), ' ', IFNULL(p.apellido_Persona,''))) AS nombre,
                p.img_Perfil AS imagen
            FROM conversaciones_miembros cm
            JOIN Personas p ON p.id_Perfil_Persona = cm.id_Perfil_Persona
            WHERE cm.id_conversacion = ?
            ORDER BY cm.rol DESC, nombre ASC
        `, [idConversacion]);
        res.json({ success: true, data: rows });
    } catch (err) {
        console.error('[GET /grupo/miembros]', err);
        res.status(500).json({ success: false, message: 'Error obteniendo miembros del grupo', error: err.message });
    }
});
