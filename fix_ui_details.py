...
roles_code = """window.abrirModalEditarPermisos = function(idRol) {
            ...
            window.acordeonesAbiertos = { 
                ...
                'Directorio de Usuarios': false,
                'Solicitudes de Verificación': false,
                'Otros Permisos': false
            };

            const MAPEO_AVANZADO = {
                'VER_METRICAS': { grupo: 'Administrador', nombre: 'Ver panel de métricas y reportes', icon: 'mdi:chart-pie', badgeText: 'VER', badgeColor: 'bg-slate-100 text-slate-700' },
                'MODERAR_USUARIOS': { grupo: 'Administrador', nombre: 'Bloquear y moderar usuarios de la comunidad', icon: 'mdi:shield-account-outline', badgeText: 'ELIMINAR', badgeColor: 'bg-red-50 text-red-600' },
                'GESTIONAR_CONFIGURACION': { grupo: 'Administrador', nombre: 'Gestionar configuración general del sistema', icon: 'mdi:cog-outline', badgeText: 'ADMIN', badgeColor: 'bg-purple-50 text-purple-600' },
                'ACEPTAR_INTERCAMBIOS': { grupo: 'Administrador', nombre: 'Aceptar y gestionar intercambios', icon: 'mdi:handshake-outline', badgeText: 'VER', badgeColor: 'bg-slate-100 text-slate-700' },
                'VER_HISTORIAL_PERSONAL': { grupo: 'Administrador', nombre: 'Ver historial de actividades', icon: 'mdi:history', badgeText: 'VER', badgeColor: 'bg-slate-100 text-slate-700' },
                'VER_DIRECTORIO': { grupo: 'Administrador', nombre: 'Ver listado y tabla del directorio general', icon: 'mdi:folder-account-outline', badgeText: 'VER', badgeColor: 'bg-slate-100 text-slate-700' },
                'CREAR_CUENTAS': { grupo: 'Administrador', nombre: 'Crear y enrolar nuevas cuentas de usuario', icon: 'mdi:account-plus-outline', badgeText: 'CREAR', badgeColor: 'bg-emerald-50 text-emerald-600' },
                'EDITAR_USUARIOS': { grupo: 'Administrador', nombre: 'Editar perfiles y datos de usuarios', icon: 'mdi:account-edit-outline', badgeText: 'EDITAR', badgeColor: 'bg-amber-50 text-amber-600' },
                'ASIGNAR_ROLES_PERMISOS': { grupo: 'Administrador', nombre: 'Asignar roles y permisos a usuarios', icon: 'mdi:shield-key-outline', badgeText: 'ADMIN', badgeColor: 'bg-purple-50 text-purple-600' },
                'VER_SOLICITUDES_VERIFICACION': { grupo: 'Administrador', nombre: 'Ver y validar solicitudes de verificación', icon: 'mdi:check-decagram-outline', badgeText: 'VER', badgeColor: 'bg-slate-100 text-slate-700' },
            ...
            window.renderizarModalPermisos = () => {
                ...
                todosLosPermisosDisponibles.forEach(p => {
                    ...
                    if (!grupos[map.grupo]) grupos[map.grupo] = { permisos: [], icon: map.grupo === 'Administrador' ? 'mdi:shield-star-outline' : (map.grupo === 'Menú de Navegación' ? 'mdi:compass-outline' : 'mdi:checkbox-marked-circle-outline') };
                ...
                const ordenGrupos = ['Menú de Navegación', 'Administrador', 'Otros Permisos'];
                ...
                for (const nombreGrupo of gruposOrdenados) {
                    ...
                    const headerBg = isOpen ? 'bg-slate-50' : 'bg-white';
                    const rightSideContent = isOpen ? `
                        <div class="flex items-center gap-6">
                            <span class="text-xs font-bold text-blue-600">${seleccionadosGrupo}/${cantTotal}</span>
                            ...
                            <span class="iconify text-slate-400 text-xl transition-transform rotate-180" data-icon="mdi:chevron-up"></span>
                        </div>
                    ` : `
                        <div class="flex items-center gap-2">
                            <span class="iconify text-slate-400 text-xl transition-transform" data-icon="mdi:chevron-down"></span>
                        </div>
                    `;

                    htmlGrupos += `
                        <div class="mb-4 rounded-xl border border-slate-200 overflow-hidden ${isOpen ? 'bg-slate-50' : 'bg-white'} transition-colors">
                            <div class="flex items-center justify-between px-6 py-4 cursor-pointer hover:bg-slate-50 transition-colors" onclick="toggleAcordeonPermisos('${nombreGrupo}')">
                                <div class="flex items-center gap-4">
                                    <div class="w-8 h-8 rounded-full bg-blue-50 flex items-center justify-center text-blue-500 shrink-0">
                                        <span class="iconify text-lg" data-icon="${data.icon}"></span>
                                    </div>
                                    <h4 class="font-bold text-slate-700 text-[15px]">${nombreGrupo}</h4>
                                </div>
                                ${rightSideContent}
                            </div>
                            <div class="transition-all ${isOpen ? 'block' : 'hidden'} bg-white border-t border-slate-100">
                                ${htmlPermisos}
                            </div>
                        </div>
                    `;
                ...
                document.getElementById('permisosCheckboxesContainer').innerHTML = `
                    <div class="flex flex-col gap-4">
                        <div class="flex items-center gap-4 border border-slate-200 rounded-xl p-1 mb-2 bg-white">
                            <div class="relative flex-1">
                                <span class="absolute inset-y-0 left-0 flex items-center pl-3 text-slate-400">
                                    <span class="iconify text-lg" data-icon="mdi:magnify"></span>
                                </span>
                                <input type="text" placeholder="Buscar permiso..." value="${escapeHtml(window.terminoBusquedaPermisos)}" oninput="buscarPermisosEnModal(this.value)" class="w-full pl-10 pr-4 py-2 bg-transparent border-none text-sm focus:outline-none focus:ring-0 text-slate-700">
                            </div>
                            <div class="w-px h-6 bg-slate-200"></div>
                            <div class="flex items-center gap-3 px-4 py-1 text-xs font-bold text-slate-500">
                            ...
                            <div class="w-px h-6 bg-slate-200"></div>
                            <div class="flex items-center gap-2 px-4 py-1 text-emerald-600 text-xs font-bold whitespace-nowrap">
                                <span class="iconify text-base" data-icon="mdi:check-all"></span>
            ...
            window.seleccionarTodoGrupo = (grupo, seleccionar) => {
                todosLosPermisosDisponibles.forEach(p => {
                    const MAPEO_AVANZADO = {
                        'VER_METRICAS': { grupo: 'Administrador' },
                        'MODERAR_USUARIOS': { grupo: 'Administrador' },
                        'GESTIONAR_CONFIGURACION': { grupo: 'Administrador' },
                        'ACEPTAR_INTERCAMBIOS': { grupo: 'Administrador' },
                        'VER_HISTORIAL_PERSONAL': { grupo: 'Administrador' },
                        'VER_DIRECTORIO': { grupo: 'Administrador' },
                        'CREAR_CUENTAS': { grupo: 'Administrador' },
                        'EDITAR_USUARIOS': { grupo: 'Administrador' },
                        'ASIGNAR_ROLES_PERMISOS': { grupo: 'Administrador' },
                        'VER_SOLICITUDES_VERIFICACION': { grupo: 'Administrador' },
            ...
            modalRoot.innerHTML = `
                ...
                <div id="permisosCard" class="relative bg-white rounded-3xl w-full max-w-[900px] max-h-[90vh] shadow-[0_20px_60px_rgb(0,0,0,0.2)] border border-slate-100 flex flex-col overflow-hidden z-[9999] transition-all m-auto">
                    <div class="flex items-center justify-between p-6 pb-2 shrink-0 bg-white">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h3 id="permisosRoleNombre" class="text-xl font-bold text-slate-800 tracking-tight">Asignar permisos al rol: ${rol.nombre_rol}</h3>
                                <p class="text-xs text-slate-400 mt-0.5 font-medium">Gestiona los accesos y privilegios de este rol.</p>
                            </div>
                        </div>
                    ...
                    <div class="px-8 py-5 bg-white border-t border-slate-100 flex justify-end gap-3 shrink-0">
                        <button type="button" onclick="cerrarModalEditarPermisos()" class="px-6 py-2.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 text-sm font-bold transition-all cursor-pointer">Cancelar</button>
...
print("Modal adjusted to perfectly match screenshot details!")
