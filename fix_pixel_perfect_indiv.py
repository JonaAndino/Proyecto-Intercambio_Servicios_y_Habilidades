...
indiv_code = """window.abrirModalPermisosIndividuales = async function(idUsuario, nombre, idRol) {
            ...
            modalRoot.innerHTML = `
                ...
                <div id="permisosIndivCard" class="relative bg-slate-50/80 backdrop-blur-md rounded-[32px] w-full max-w-[950px] max-h-[90vh] shadow-[0_20px_60px_rgb(0,0,0,0.15)] border border-slate-200/50 flex flex-col overflow-hidden z-[9999] transition-all m-auto">
                    <div class="flex items-center justify-between px-8 py-6 shrink-0">
                        ...
                        <button onclick="cerrarModalPermisosIndividuales()" class="w-8 h-8 flex items-center justify-center rounded-full bg-slate-100 hover:bg-slate-200 text-slate-500 transition-colors cursor-pointer" aria-label="Cerrar modal">
            ...
            try {
                ...
                window.renderizarModalPermisosIndiv = () => {
                    ...
                    for (const nombreGrupo of gruposOrdenados) {
                        ...
                        const concedidosGrupo = data.permisos.filter(p => {
                        ...
                        const hasActive = concedidosGrupo > 0;
                        ...
                        htmlGrupos += `
                            <div class="mb-4 rounded-2xl bg-white border border-slate-100 shadow-sm overflow-hidden transition-all">
                                <div class="flex items-center justify-between px-6 py-4 cursor-pointer hover:bg-slate-50 transition-colors border-b border-transparent ${isOpen ? 'border-slate-100 bg-slate-50/50' : ''}" onclick="window.acordeonesAbiertosIndiv['${nombreGrupo}'] = !window.acordeonesAbiertosIndiv['${nombreGrupo}']; window.renderizarModalPermisosIndiv();">
                                    <div class="flex items-center gap-3">
                                        <h4 class="font-bold text-slate-700 text-[15px]">${nombreGrupo}</h4>
                                        ${hasActive ? '<div class="w-2 h-2 rounded-full bg-emerald-500 shadow-sm"></div>' : ''}
                                    </div>
                        ...
                        <div class="flex flex-col gap-4">
                            <div class="flex items-center gap-4 border border-slate-200 rounded-2xl p-1.5 mb-2 bg-white shadow-sm">
                                ...
                                <div class="flex items-center gap-3 px-4 py-1 text-[13px] font-bold text-slate-600">
                                    <button onclick="window.excepcionesCargadas.forEach(e => e.concedido = true); window.renderizarModalPermisosIndiv();" class="hover:text-slate-900 transition-colors">Todo</button>
                                    <span class="w-px h-3 bg-slate-300"></span>
                                    <button onclick="window.excepcionesCargadas.forEach(e => e.concedido = false); window.renderizarModalPermisosIndiv();" class="hover:text-slate-900 transition-colors">Nada</button>
                                </div>
                                <div class="w-px h-6 bg-slate-200"></div>
...
print("Permisos Individuales Modal EXACT match with green dot applied!")
