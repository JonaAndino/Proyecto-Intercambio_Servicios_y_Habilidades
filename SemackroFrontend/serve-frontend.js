...
// ── PROXY INTERNO al backend ────────────────────────────────────────────────
// El navegador llama a https://ngrok-url/api/* o /socket.io/*
// Este servidor reenvía esas peticiones a localhost:3001 internamente.
// Así el navegador NUNCA hace requests directos a localhost → sin advertencia Chrome.
function proxyRequest(req, res) {
  ...
  const proxyReq = http.request(options, (proxyRes) => {
    // Agregar cabeceras CORS para el cliente ngrok
...
const server = http.createServer((req, res) => {
  try {
    ...
    const csp = "default-src 'self' data:; " +
            ...
            "connect-src 'self' http://127.0.0.1:5050 http://localhost:5050 http://127.0.0.1:3001 http://localhost:3001 http://localhost:5678 http://127.0.0.1:5678 https://SEMACKRO.duckdns.org http://SEMACKRO.duckdns.org wss://SEMACKRO.duckdns.org ws: wss: https://meet.jit.si wss://meet.jit.si https://*.jitsi.net wss://*.jitsi.net https://8x8.vc https://*.8x8.vc wss://8x8.vc wss://*.8x8.vc https://alpha.jitsi.net wss://alpha.jitsi.net https://cdn.jsdelivr.net https://cdn.socket.io https://unpkg.com https://cdnjs.cloudflare.com https://nominatim.openstreetmap.org https://*.tile.openstreetmap.org https://api.iconify.design https://api.unisvg.com https://api.simplesvg.com https://*.googleapis.com https://generativelanguage.googleapis.com https://api-inference.huggingface.co https://*.firebase.com https://*.firebaseio.com https://identitytoolkit.googleapis.com https://securetoken.googleapis.com https://www.gstatic.com;";
    res.setHeader('Content-Security-Policy', csp);

    const decoded = decodeURIComponent(req.url.split('?')[0]);
    
    // Rutas de API y Socket.IO → proxy al backend (sin salir al navegador)
    if (decoded.startsWith('/api/') || decoded.startsWith('/socket.io')) {
...
server.listen(PORT, () => {
  ...
  console.log(`🔀 Proxy de API activo → localhost:${BACKEND_PORT}`);
  console.log(`📁 Sirviendo carpeta: ${PUBLIC_DIR}`);
  console.log(`🛡️  Sin advertencias Chrome: el navegador NO llama directamente a localhost`);
});

// Proxy WebSocket para Socket.IO
server.on('upgrade', (req, socket, head) => {
  const net = require('net');
  const backendSocket = net.connect(BACKEND_PORT, 'localhost', () => {
    backendSocket.write(
