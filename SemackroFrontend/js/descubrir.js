...
async function navigateTo(viewName) {
  ...
  if (viewName === "descubrir" && !window._dashboardInicializado) {
    window._dashboardInicializado = true;
...
// Datos de sesión del usuario (nombre, foto) siempre se cargan si hay sesión activa
const _token = sessionStorage.getItem('token') || localStorage.getItem('token');
if (_token && localStorage.getItem('usuarioId')) {
    cargarDatosUsuario();
}

// Llamar a la función de inicialización solo si el usuario puede ver la vista Descubrir
// Esto evita peticiones innecesarias al servidor cuando el usuario será redirigido
(function() {
    const permisos = window.Permisos ? window.Permisos._obtenerPermisos() : [];
    const esAdmin = sessionStorage.getItem('usuarioRolId') === '1' || localStorage.getItem('usuarioRolId') === '1';
    // Si no hay permisos definidos (usuario estándar o legacy admin), cargar normalmente
    // Si hay permisos definidos, solo cargar si tiene acceso a 'descubrir'
    if (permisos.length === 0 || permisos.includes('descubrir') || esAdmin) {
        window._dashboardInicializado = true;
        inicializarDashboard();
    } else {
        console.log('[OPTIMIZACIÓN] Saltando carga de Descubrir: usuario sin permiso para esta vista');
    }
})();
